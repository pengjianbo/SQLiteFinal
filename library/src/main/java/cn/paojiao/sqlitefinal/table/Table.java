...
public class Table {
    ...
    private Table(DbHelper db, Class<?> entityType) {
        this.db = db;
    ...
    public static synchronized Table get(DbHelper db, Class<?> entityType) {
    ...
    public static synchronized void remove(DbHelper db, Class<?> entityType) {
        String tableKey = db.getDaoConfig().getDbName() + "#" + entityType.getName();
        tableMap.remove(tableKey);
    }

    public static synchronized void remove(DbHelper db, String tableName) {
        if (tableMap.size() > 0) {
            String key = null;
